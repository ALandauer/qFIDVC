...
sizeB = size(cur_bead_img); %should be n x n x n, n=bead_size_small
...
parfor bead_num = 1:size(x1,1)
    ...
    cur_bead_shift{bead_num} = interpn(px_x+center_offset(1),px_y+center_offset(2),...
        px_z+center_offset(3),cur_bead_img,px_x,px_y,px_z,'spline');
end
...
for bead_num = 1:size(x1,1)
    ...
    composed_img(center_int(1)-fix(bead_size_small/2):center_int(1)+floor(bead_size_small/2),...
        ...
        center_int(3)-fix(bead_size_small/2):center_int(3)+floor(bead_size_small/2))...
        + ...
        cur_bead_shift{bead_num};%(2:end,2:end,2:end);
...
%     toc(t2)
    ...
    %     tic
    %     cbi = gpuArray(cur_bead_img);
    %     cur_bead_shift = gather(interpn(px_x+center_offset(1),px_y+center_offset(2),...
    %         px_z+center_offset(3),cbi,px_x,px_y,px_z,'linear'));
    %     toc
