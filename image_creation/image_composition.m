...
sizeI = [2047,511,15]; %final image size less one
spacing = 4; %partical min. spacing
nPts = round(6*16000000/6.5); %number of particals
try
...
sn = 10; %signal to noise ratio, 10 compares reasonably to exp images.
bl = 15; %black level, i.e. background brightness onto which,
...
% translation = [0:0.2:2];
int_decrement = linspace(1.0,0.75,length(stretch_lvls));
...
for stretch = 1:length(stretch_lvls)
    ...
    %seed in beads
    %     tic
    I{stretch} = seedDefBeads(small_bead,sizeI,x1,bead_size);
    %     toc
    ...
    %
    I0 = I0_;
    save(['./high_density2',num2str(nPts),'beads_0p01step_lambda_',num2str(cur_stretch),'.mat'],'I0')
    ...
    save(['./high_density2',num2str(nPts),'beads_noisy_0p01step_lambda_',...
